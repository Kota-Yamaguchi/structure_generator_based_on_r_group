...
def generate_structure():
    with st.spinner("モデルをロード中..."):
        ...
        target_ranges = pd.read_csv(
            "settings_of_target_ys.csv", index_col=0
        )  # 各 y の目標範囲の読み込み
        ...
        original_x = pd.DataFrame(
            descriptors, index=dataset.index, columns=descriptor_names
        )
        if deleting_descriptor_names is not None:
            original_x = original_x.drop(deleting_descriptor_names, axis=1)
        original_x = original_x.replace(np.inf, np.nan).fillna(
            np.nan
        )  # inf を NaN に置き換え
        nan_variable_flags = original_x.isnull().any()  # NaN を含む変数
        original_x = original_x.drop(
            original_x.columns[nan_variable_flags], axis=1
        )  # NaN を含む変数を削除
        # 標準偏差が 0 の説明変数を削除
        ...
        autoscaled_target_y_value = (
            target_y_value - variables.mean(axis=0)[numbers_of_y]
        ) / variables.std(axis=0, ddof=1)[numbers_of_y]
    ...
    with st.spinner("構造を生成中..."):
        if model.success_flag:
            ...
            (
                ...
                mode_of_estimated_mean_of_y,
                responsibilities_y,
                py,
            ) = model.gtmr_predict(
                autoscaled_variables.iloc[:, numbers_of_x], numbers_of_x, numbers_of_y
            )

            plt.rcParams["font.size"] = 18
            for index, y_number in enumerate(numbers_of_y):
                predicted_y_test = (
                    mode_of_estimated_mean_of_y[:, index]
                    * variables.iloc[:, y_number].std()
                ...
                y_max = np.max(
                    np.array([np.array(variables.iloc[:, y_number]), predicted_y_test])
                )
                y_min = np.min(
                    np.array([np.array(variables.iloc[:, y_number]), predicted_y_test])
                )
                ...
                print(
                    "r2: {0}".format(
                        float(
                            1
                            - sum((variables.iloc[:, y_number] - predicted_y_test) ** 2)
                            / sum(
                                (
                ...
                print(
                    "RMSE: {0}".format(
                        float(
                            (
                                sum(
                                    (variables.iloc[:, y_number] - predicted_y_test)
                                    ** 2
                                )
                                / len(variables.iloc[:, y_number])
                            )
                            ** 0.5
                        )
                    )
                ...
                print(
                    "MAE: {0}".format(
                        float(
                            sum(abs(variables.iloc[:, y_number] - predicted_y_test))
                            / len(variables.iloc[:, y_number])
                        )
                    )
                )
            ...
            (
                autoscaled_mean_of_estimated_mean_of_x,
                autoscaled_mode_of_estimated_mean_of_x,
                responsibilities_y,
                py,
            ) = model.gtmr_predict(
                autoscaled_target_y_value, numbers_of_y, numbers_of_x
            )

            # Check results of inverse analysis
            print("Results of inverse analysis")
            mean_of_estimated_mean_of_x = pd.DataFrame(
                autoscaled_mean_of_estimated_mean_of_x, columns=x.columns
            ) * x.std(axis=0, ddof=1) + x.mean(axis=0)
            mode_of_estimated_mean_of_x = pd.DataFrame(
                autoscaled_mode_of_estimated_mean_of_x, columns=x.columns
            ) * x.std(axis=0, ddof=1) + x.mean(axis=0)
            ...
            plt.scatter(
                estimated_x_mode_on_map[0],
                estimated_x_mode_on_map[1],
                c="red",
                marker="x",
                s=100,
            )
        ...
        def evalOneMax(individual):
            ...
            generated_smiles = structure_generator.structure_generator_based_on_r_group(
                main_molecules, fragment_molecules, individual_array
            )
            generated_molecule = Chem.MolFromSmiles(generated_smiles)
            if generated_molecule is not None:
                AllChem.Compute2DCoords(generated_molecule)
                descriptors_of_generated_molecule = (
                    descriptor_calculator.CalcDescriptors(generated_molecule)
                )
                descriptors_of_generated_molecule = pd.DataFrame(
                    descriptors_of_generated_molecule, index=descriptor_names
                )
                descriptors_of_generated_molecule = descriptors_of_generated_molecule.T
                if deleting_descriptor_names is not None:
                    descriptors_of_generated_molecule = (
                        descriptors_of_generated_molecule.drop(
                            deleting_descriptor_names, axis=1
                        )
                    )
                descriptors_of_generated_molecule = (
                    descriptors_of_generated_molecule.drop(
                        descriptors_of_generated_molecule.columns[nan_variable_flags],
                        axis=1,
                    )
                )  # NaN を含む変数を削除
                descriptors_of_generated_molecule = (
                    descriptors_of_generated_molecule.drop(
                        descriptors_of_generated_molecule.columns[std_0_variable_flags],
                        axis=1,
                    )
                )
                descriptors_of_generated_molecule = (
                    descriptors_of_generated_molecule.replace(np.inf, np.nan).fillna(
                        np.nan
                    )
                )  # inf を NaN に置き換え
                if descriptors_of_generated_molecule.isnull().sum(axis=1)[0] > 0:
                    ...
                    autoscaled_x_prediction = (
                        descriptors_of_generated_molecule - x.mean()
                    ) / x.std()
                    distance = (
                        (
                            (
                                autoscaled_mode_of_estimated_mean_of_x[0, :]
                                - autoscaled_x_prediction.values[0, :]
                            )
                            ** 2
                        ).sum()
                    ) ** 0.5
        ...
        for iteration_number in range(number_of_iteration_of_ga):
            ...
            for each_pop in pop:
                if each_pop.fitness.values[0] > minimum_number / 2:
                    ...
                    smiles = structure_generator.structure_generator_based_on_r_group(
                        main_molecules, fragment_molecules, each_pop_array
                    )
                    generated_smiles_all.append(smiles)

        estimated_y_all = pd.DataFrame(
            estimated_y_all, index=generated_smiles_all, columns=["estimated_y"]
        )
        estimated_y_all = estimated_y_all.loc[
            ~estimated_y_all.index.duplicated(keep="first"), :
        ]  # 重複したサンプルの最初だけを残す
        # estimated_y_all.to_csv("generated_molecules.csv")
        # estimated_y_allyを画面に表示したい
        print(estimated_y_all)
        return estimated_y_all
    st.success("構造の生成が完了しました。")
